...
export class ManagerProfessionsRepository extends Repository<ProfessionsEntity>{
    ...
    async getAll(dto:PaginationRequestDto,lang:LangEnum) {
        ...
        .select(['professions.id','professions.created_at','professions.slug',`professions.name_tk`,`professions.desc_tk`,`professions.name_ru`,`professions.desc_ru`,`professions.name_en`,`professions.desc_en`])
        if(dto.keyword != '') {
            query.andWhere(
                new Brackets(qb => {
        ...
        return await query.take(dto.limit).skip(offset).getManyAndCount()
    }
}